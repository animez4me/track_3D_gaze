...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber ork_cloud_subscriber = n.subscribe("/kinect2/qhd/points",1,&point_cloud_callback);
  ros::Duration(1).sleep();
  ros::spinOnce();
//  image_color_sub_ = it_.subscribe("/kinect2/qhd/image_color_rect",1,imageColorCb);


    RGBDcamera kinect(RGBD_CAMERA_intrinsics, cloud);
...
//    cv::cvtColor(kinect.RGBFrame, briskf.imgGray1, CV_BGR2GRAY);
    ...
    if (show_kinect) {
        ...
        cv::imshow("color", kinect_RGBFrame);
        cv::waitKey(1);
        pcl::visualization::PointCloudColorHandlerRGBField<pcl::PointXYZRGB> rgb(cloud);
        kinect.viewer->updatePointCloud<pcl::PointXYZRGB>(cloud, rgb, "sample cloud");
